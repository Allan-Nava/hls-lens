...
| [Low latency](#low-latency) | ⏳ planned | 0/4 |
...
The LL-HLS tags are parsed and documented, and no rule reads a single one of them: a manifest can declare partial segments no player will ever manage to use, and the extension says nothing. Every item here is still computable from the declarations alone — what a part *contains* stays segcheck's job.
