...
**84 rules, on the line you have to fix**, while you edit:
...
- **Quick fixes for the mechanical findings.** Bump `EXT-X-VERSION` to what the playlist already uses, append a missing `EXT-X-ENDLIST`, raise `EXT-X-TARGETDURATION` to the longest segment, change a misspelled tag to the one it was meant to be, set `AUTOSELECT=YES` on a default rendition, drop a stray `FORCED`. Only those: a fix that needs a judgement call is not offered, and a tag more than two edits from a real one is a vendor extension rather than a typo, so it gets no guess.
...
- **This manifest against another one.** `HLS Lens: Compare With…` takes a path or a URL and says what changed: rungs added, removed or re-rated, rendition groups that came and went, a target duration that moved. Rungs are matched by **URI**, because a packager keeps the path of a rendition stable far more often than its bitrate — which is what tells "the same rung, re-rated" apart from "a new rung". A text diff answers the same question in a form nobody can read. It works on `.mpd` files too, matching periods and representations by `@id`.
- **A report you can send someone.** `HLS Lens: Export Findings as a Report` turns the findings — of the open manifest, or of the last workspace scan — into markdown for a ticket or JSON for whatever reads it next. The JSON carries a schema number so a consumer has something to pin, and lines are 1-based in both: 0-based is an editor's convention, and a report is read by people and by CI.
...
- **DASH, read the same way.** Open an `.mpd` and it gets the same treatment as a playlist: **the tree** (periods, adaptation sets and their representations with bitrate, resolution and frame rate), a status bar line, comparison against another manifest, **the timeline** drawn from its `<SegmentTimeline>` with the period boundaries marked, **document links** on the URLs it holds, **the DASH vocabulary on hover**, and fourteen `dash/*` rules reporting on it — including three that read the periods **against each other**: a codec that changes across a boundary (a decoder reset mid-presentation), a track that stops existing there, periods that do not run back to back — a `@mediaPresentationDuration` the segment timeline does not fill, `<S>` elements that do not chain (a hole in the presentation, or two segments claiming the same seconds), a dynamic manifest with no `<UTCTiming>` for clients to synchronise their clock to, an adaptation set that never declares `@segmentAlignment`, a `@media` template with no `$Number$`. The XML reader is written here, like everything else: still no dependencies.
...
| `hlsLens.diagnostics.severity` | `{}` | Re-grade a rule or a category: `{"master/ladder-spacing": "warning"}` |
| `hlsLens.diagnostics.profile` | `none` | `apple` or `low-latency`: a house style, graded under your own settings |
