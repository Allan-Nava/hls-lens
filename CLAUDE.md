...
- **Todo → `BACKLOG.md`** (sorgente unica, id stabili `HL-n`). Non sparpagliare TODO nei commenti. **Il formato è load-bearing**: `## Milestone` → milestone GitHub, `### Area` → raggruppamento, `- [ ] **HL-n — Titolo**: descrizione` → issue. Chiudere una voce = `- [x]`, mai cancellarla. Dopo ogni modifica al backlog: `npm run roadmap` (rigenera `docs/ROADMAP.md`, gate in CI). Le issue su GitHub sono un mirror generato: si edita il file, non l'issue.
