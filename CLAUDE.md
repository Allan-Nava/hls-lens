...
- `src/core/analyze.ts` — `applySeverityOverrides` (rigrada per id o per categoria; **prima** del filtro `minSeverity`, e ri-ordina) + le regole su singolo file (61) + le 9 `cross/*` documentate qui e implementate in `crosscheck.ts` + `RULES` (il catalogo documentato) + la tabella `VERSION_REQUIREMENTS` tag→versione minima. Ordine dei finding: severità, poi riga.
- `src/core/ladder.ts` — modello dell'albero (`buildLadder`, `renditionRows`, `lowLatencyRows`, `ladderSummary`) e formattazione (`formatBandwidth`, `formatResolution`). `lowLatencyRows` tronca a 50 parti e **lo dichiara** in una riga sua.
...
- `src/core/compare.ts` — `compareManifests`/`describeComparison`: due manifest confrontati come **dichiarazioni**, non come testo. I rung si appaiano per **URI** (un packager tiene stabile il path di una rendition molto più del suo bitrate, quindi l'URI è ciò che distingue "stesso rung, ri-tarato" da "rung nuovo"). Le righe dei change puntano al manifest **aperto**, non all'altro.
- `src/core/profiles.ts` — `PROFILES`/`profileOverrides`: `apple` e `low-latency` come set di severità. Si applicano **sotto** a `hlsLens.diagnostics.severity`: un profilo è un punto di partenza, non una policy.
- `src/core/report.ts` — i finding come documento: `renderFindingsMarkdown` (le pipe nei messaggi sono escapate, altrimenti un URI con dentro una `|` spezza la colonna) e `renderFindingsJson` (campo `schema`, per chi lo parsa). **Le righe sono 1-based nell'export**: lo 0-based è una convenzione dell'editor, un report lo leggono persone e CI. Nessun timestamp nel core: lo passa la glue in `subtitle`, così resta deterministico.
...
- `src/core/fixes.ts` — `quickFixesFor`: solo i finding meccanici (versione, ENDLIST, target duration, tag scritto male, `AUTOSELECT`, `FORCED`) come edit di riga; la glue li trasforma in `WorkspaceEdit`. La distanza di edit per il tag scritto male è **2**: oltre non è un typo ma un'estensione di vendor, e riscriverla distruggerebbe una riga voluta.
...
- **Un `Request timeout` del Marketplace non è un fallimento del publish**: la richiesta può essere stata accettata lo stesso. Per questo il retry tratta *already exists* come successo — è `v0.11.0` che ha insegnato la cosa, fallendo e non arrivando mai agli utenti. **Le action girano su Node 22** (`checkout@v5`, `setup-node@v5`, `upload`/`download-artifact@v5`): le v4 giravano su Node 20, deprecato e già forzato a Node 24 dai runner.
