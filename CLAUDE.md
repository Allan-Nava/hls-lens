...
- `src/core/analyze.ts` — le regole su singolo file (53) + le 9 `cross/*` documentate qui e implementate in `crosscheck.ts` + `RULES` (il catalogo documentato) + la tabella `VERSION_REQUIREMENTS` tag→versione minima. Ordine dei finding: severità, poi riga.
