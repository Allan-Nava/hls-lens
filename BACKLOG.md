...
Eighty-four rules read what a manifest *declares*, and a manifest can be perfect and
point at nothing. The most common failure in production is not a malformed tag: it is a
rung that 404s, an init segment behind a login page that answers `200 text/html`, or a
live playlist a CDN is caching for an hour. None of that needs the segment bytes —
which is segcheck's job — only the response headers, so it belongs here.

The network makes this the first feature that cannot be tested on a fixture alone. It
is tested the way the fetcher already is: a throwaway `http` server on a random local
port. A test that touches a CDN is still a bug.
